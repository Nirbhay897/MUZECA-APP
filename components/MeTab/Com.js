...
const Com = () => {
  ...
  return (
    <ScrollView className='flex'>
        {/* {datas.map((dat, index)=>(
            <ComCard key={index} title={dat.title} imageurl={dat.icon} caption={dat.caption} index={index} />
        ))} */}

        {playlist.map((item, index)=>(
          <ComCard key={index} title={item.title} imageurl={item.imageurl} caption={item.audios.length > 1 ? `${item.audios.length} songs` : `${item.audios.length} song`} index={index} />
        ))}
